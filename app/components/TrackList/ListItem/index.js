...
const ListItem = ({ item, onPressItem }) => {
  const { id, elapsedTime, startEndDistance, date } = item;

  return (
    <TouchableHighlight style={styles.container} onPress={() => onPressItem({ id })}>
      <View style={styles.item}>
        <Icon name="ios-map" size={20} />
        <Text>{format(date, 'ddd DD/MM/YYYY HH:mm')}</Text>
        <Text>{`${secondsToHHMMSS(elapsedTime)}`}</Text>
        <Text>{`${startEndDistance && startEndDistance.toFixed(1)} km`}</Text>
      </View>
    </TouchableHighlight>
  );
};
